Citizen.CreateThread(function()
	SetMapName('FOX WORLD')
	SetGameType('FOX WORLD')
end)
		...
		if ESX.GetPlayerFromIdentifier(identifier) then
			...
			MySQL.Async.fetchScalar('SELECT 1 FROM users WHERE identifier = @identifier', {
				...
				if result then
					...
					MySQL.Async.execute('INSERT INTO users (`name`, `accounts`, `identifier`, `group`) VALUES (@name, @accounts, @identifier, @group)', {
...
ESX.RegisterServerCallback('ESX:HasItem', function(source, cb, items, amount)
    local src = source
    local retval = false
...
RegisterNetEvent('esx:save', function(xPlayer)
	local src = source
	local Player = ESX.GetPlayerFromId(src)
	local xPlayer = xPlayer or Player

...
ESX.ShowNotification = function(msg)
	...
	AddTextComponentString("<FONT FACE='Balo'>"..msg.."</FONT>")
...
ESX.ShowHelpNotification = function(msg, thisFrame, beep, duration)
	AddTextEntry('esxHelpNotification', "<FONT FACE='Balo'>"..msg.."</FONT>")
...
ESX.Notify = function(message, type, title, time)
	local message = message or ''
    local type = type or 'info'
    local title = title or 'Thông Báo'
    local time = time or 5000

	exports['esx_notify']:Alert(message, type, title, time)
end
...
ESX.HasItem = function(item)
    local p = promise.new()
	ESX.TriggerServerCallback('ESX:HasItem', function(result)
        if result then
            p:resolve(true)
        end
        p:resolve(false)
...
ESX.GetClosestVehicle = function(coords)
